...
const createCrisis = async (req, res) => {
  try {
    const { name ,place, ngoId, description, peopleAffected, situationRating, fundsRequired } = req.body;
    ...
    return res.status(201).json({ message: 'Crisis created successfully.', crisis: newCrisis });
  } catch (error) {
    console.error(error);
